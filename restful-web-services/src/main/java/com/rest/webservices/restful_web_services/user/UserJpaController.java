...
//	WebMvcLinkBuilder
	...
	@GetMapping(path = "/jpa/users/{ids}/posts/{id}")
	public Post retrivePostForUser(@PathVariable int ids, @PathVariable int id) {
		Optional<User> user = userRepository.findById(ids);
		if (user.isEmpty()) {
			throw new UserNotFoundException("id " + ids);
		}
		List<Post> userPost = user.get().getPost();
